...
class LASNet(nn.Module):
    def __init__(self, n_classes=1,ind=50):
        ...
        self.rgb_encoder=Res2Net_model(ind)
        self.depth_encoder: nn.Module = timm.create_model(
            model_name="vgg16", features_only=True, out_indices=range(0, 6)
        )
        self.decoder = prediction_decoder(64,256,512,1024,2048, n_classes)
        self.depth_encoder.load_state_dict(torch.load('./pretrained/vgg16.pth', map_location="cpu"), strict=False)
        self.conv_128_64 = nn.Conv2d(128, 64, kernel_size=1)
        self.conv_512_1024 = nn.Conv2d(512, 1024, kernel_size=1)
        self.conv_512_2048 = nn.Conv2d(512, 2048, kernel_size=1)
    ...
    def forward(self, rgb, depth,training = True):
        ...
        t_dict_5 = self.conv_512_2048(t_dict[5])
        t_dict_4 = self.conv_512_1024(t_dict[4])
        t_dict_1 = self.conv_128_64(F.interpolate(t_dict[1], size=(120, 160), mode='bilinear', align_corners=False))
        
        x4 = rgb_dict[4] + t_dict_5
        x3 = rgb_dict[3] + t_dict_4
        x2 = rgb_dict[2] + t_dict[3]
        x1 = rgb_dict[1] + t_dict[2]
        x0 = rgb_dict[0] + t_dict_1
        ...
        if training:
            semantic_Depth, semantic2_Depth, x5_decoder_Depth, x4_decoder_Depth, x3_decoder_Depth, x2_decoder_Depth  = self.decoder(t_dict_5,t_dict_4,t_dict[3],t_dict[2],t_dict_1)
            Depth_decoder = [x5_decoder_Depth, x4_decoder_Depth, x3_decoder_Depth, x2_decoder_Depth]
            return semantic, semantic2, rgb_dict[4], t_dict_4, RGBD_decoder, Depth_decoder
