...
class LASNet(nn.Module):
    def __init__(self, n_classes=1,ind=101):
        ...
        self.rgb_encoder: nn.Module = timm.create_model(
            model_name="resnet152d", features_only=True, out_indices=range(0, 5)
        )
        self.depth_encoder: nn.Module = timm.create_model(
            model_name="resnet152d", features_only=True, out_indices=range(0, 5)
        )
        
        self.rgb_encoder.load_state_dict(torch.load('./pretrained/resnet152d.pth', map_location="cpu"), strict=False)
        self.depth_encoder.load_state_dict(torch.load('./pretrained/resnet152d.pth', map_location="cpu"), strict=False)
