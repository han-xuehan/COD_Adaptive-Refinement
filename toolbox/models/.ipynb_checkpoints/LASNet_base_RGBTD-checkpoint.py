...
class LASNet(nn.Module):
    def __init__(self, n_classes=1,ind=50):
        ...
        self.backbone2 =Res2Net_model(ind)
    ...
    def forward(self, rgb, depth, thermal):
        x = rgb
        dp = depth[:, :1, ...]
        dp = torch.cat((dp, dp, dp), dim=1) 
        th = thermal[:, :1, ...]
        th = torch.cat((th, th, th), dim=1) 
        
        rgb_dict = self.backbone(x)
        d_dict = self.backbone1(dp)
        t_dict = self.backbone2(th)
        
        x4 = rgb_dict[4] + d_dict[4]+ t_dict[4]
        x3 = rgb_dict[3] + d_dict[3]+ t_dict[3]
        x2 = rgb_dict[2] + d_dict[2]+ t_dict[2]
        x1 = rgb_dict[1] + d_dict[1]+ t_dict[1]
        x0 = rgb_dict[0] + d_dict[0]+ t_dict[0]
        ...
        return semantic, semantic2, rgb_dict[4], d_dict[4], t_dict[4]
