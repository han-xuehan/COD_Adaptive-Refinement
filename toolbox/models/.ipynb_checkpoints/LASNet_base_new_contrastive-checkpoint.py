...
class prediction_decoder(nn.Module):
    ...
    #def __init__(self, channel1=64*2, channel2=256*2, channel3=512*2, channel4=1024*2, channel5=2048*2, n_classes=5):
        ...
        self.decoder_for_loss_computing_rgb = nn.Sequential(
                nn.Dropout2d(p=0.1),
                BasicConv2d(channel5, channel5, kernel_size=3, padding=3, dilation=3),
                #LiteMLA(channel5, channel5),
                BasicConv2d(channel5, n_classes, kernel_size=3, padding=1),
                nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
                )
        
        self.decoder_for_loss_computing_thermal = nn.Sequential(
                nn.Dropout2d(p=0.1),
                BasicConv2d(channel5, channel5, kernel_size=3, padding=3, dilation=3),
                #LiteMLA(channel5, channel5),
                BasicConv2d(channel5, n_classes, kernel_size=3, padding=1),
    ...
    def forward(self, x5, x4, x3, x2, x1, RGB, thermal):
        ...
        RGB_loss_encoder=self.decoder_for_loss_computing_rgb(RGB)
        RGB_loss_encoder_resized = F.interpolate(RGB_loss_encoder, size=(480, 640), mode='bilinear', align_corners=False)
        
        Thermal_loss_encoder=self.decoder_for_loss_computing_thermal(thermal)
        Thermal_loss_encoder_resized = F.interpolate(Thermal_loss_encoder, size=(480, 640), mode='bilinear', align_corners=False)
    
        return semantic_pred,semantic_pred2, RGB_loss_encoder_resized, Thermal_loss_encoder_resized
...
class LASNet(nn.Module):
    ...
    def forward(self, rgb, depth):
        ...
        #print(rgb_dict["stage0"].shape)     #torch.Size([4, 16, 240, 320])
        #print(rgb_dict["stage1"].shape)      # torch.Size([4, 32, 120, 160])
        #print(rgb_dict["stage2"].shape)      #torch.Size([4, 64, 60, 80])
        #print(rgb_dict["stage3"].shape)     #torch.Size([4, 128, 30, 40])
        #print(rgb_dict["stage4"].shape)     #torch.Size([4, 256, 15, 20])
        #print(sjh)
        ...
        #print(x4.shape)
        #print(s)
        semantic, semantic2, RGB_loss_encoder_resized, Thermal_loss_encoder_resized = self.decoder(x4,x3,x2,x1,x0,rgb_dict[4],t_dict[4])
        semantic2 = torch.nn.functional.interpolate(semantic2, scale_factor=2, mode='bilinear')

        
        return semantic, semantic2, rgb_dict[4], t_dict[4], RGB_loss_encoder_resized, Thermal_loss_encoder_resized
