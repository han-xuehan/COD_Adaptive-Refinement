...
class RGBD(data.Dataset):
    ...
    def __init__(self, cfg, mode,do_aug=True):
       ...
       # if cfg['class_weight'] == 'enet':
        ...
        if mode == 'train':
            ...
            img_path = os.path.join(self.root, 'test_images/')       
        ...
        if mode == 'train':
            ...
            self.depth_orginal = [(os.path.join(self.root, 'Depth_Original', img_name +'.png')) for img_name in img_list]
            self.depth_generated = [(os.path.join(self.root, 'Depth_Generated', img_name +'.png')) for img_name in img_list]
            self.gts = [(os.path.join(self.root, 'GT', img_name + '.png')) for img_name in img_list]
            
        else:
           
            self.images = [(os.path.join(self.root,'test_images' ,img_name +'.jpg')) for img_name in img_list]
            self.depth_orginal = [(os.path.join(self.root, 'test_depth', img_name +'.png')) for img_name in img_list]
            self.depth_generated = [(os.path.join(self.root, 'Depth_Generated', img_name +'.png')) for img_name in img_list]
            self.gts = [(os.path.join(self.root, 'test_masks', img_name + '.png')) for img_name in img_list]
        ...
        self.depth_generated = sorted(self.depth_generated)
        self.depth_orginal = sorted(self.depth_orginal)
    ...
    def __getitem__(self, index):
        ...
        depth_generated = Image.open(self.depth_generated[index]).convert('RGB')
        depth_orginal = Image.open(self.depth_orginal[index]).convert('RGB')
...
#        binary_label = Image.open(self.gts[index])
        ...
        sample['depth_generated'] = self.th_to_tensor(sample['depth_generated'])
        sample['depth_orginal'] = self.th_to_tensor(sample['depth_orginal'])
