...
def scoremap2bbox(scoremap, threshold, multi_contour_eval=False):
    ...
    #对热力图进行二值化处理。cv2.threshold 将 scoremap_image 中高于 threshold 的值设置为 255（白色），其他的设置为 0（黑色）
    _, thr_gray_heatmap = cv2.threshold(
        src=scoremap_image,
        thresh=int(threshold * np.max(scoremap_image)),
        maxval=255,
        type=cv2.THRESH_BINARY)
    #使用 cv2.findContours 函数在二值图像中查找轮廓
    ...
    #return box, cnt
    '''
    aff_mask = torch.zeros((scoremap.shape[0], scoremap.shape[1]))
    for i_ in range(len(contours)):
        x0_, y0_, x1_, y1_ = np.asarray(estimated_boxes)[i_]
        aff_mask[y0_:y1_, x0_:x1_] = 1

    points = np.column_stack(np.where(aff_mask > 0))
    # 获取边界框
    x_min, y_min = np.min(points, axis=0)
    x_max, y_max = np.max(points, axis=0)
    binary_map = np.zeros((scoremap.shape[0], scoremap.shape[1], 3))
    cv2.rectangle(binary_map, (y_min, x_min), (y_max, x_max), (255, 255, 255), -1)
    binary_map = binary_map.astype(np.uint8)
    #binary_map = cv2.resize(binary_map, (256, 256))
    gray_map = cv2.cvtColor(binary_map, cv2.COLOR_BGR2GRAY)
